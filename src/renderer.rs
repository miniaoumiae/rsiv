...
pub fn clear(frame: &mut [u8], color: (u8, u8, u8)) {
    // Process 4-byte chunks in parallel
...
pub fn draw_image(frame: &mut [u8], buf_w: i32, buf_h: i32, params: &DrawImageParams) {
    ...
    let end_y = (tl_y + scaled_h).min(buf_h as f64) as i32; // Use buf_h here, handle status bar clipping via buffer size
    ...
    // Pre-calculate X start offset to avoid calculating it in every thread
    let global_src_x_start_f = (start_x as f64 - tl_x) * inv_scale;

    // Split the frame into rows (buf_w * 4 bytes per row)
    // We iterate over ALL rows in the buffer, but only process those within y-bounds
    frame
        ...
        .for_each(|(y, row_pixels)| {
            ...
            if src_y >= 0 && src_y < src_height {
                ...
                // Slice the row to just the area we are drawing to avoid 'if' checks inside
